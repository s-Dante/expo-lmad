...
@php
    ...
    $materiasUrl = route('admin.teachers.materias.sync', $profesor);
    $materiaIds = $profesor->materias->pluck('id')->toJson();
    $materiaCount = $profesor->materias->count();
@endphp
...
<div class="expo-card card-c">
    ...
    <div class="card-data">
        ...
        <div class="card-info-name">
            ...
            <div class="card-visitor">
                ...
                <p>Materias: <span>{{ $materiaCount }}</span></p>
            </div>
        </div>
    </div>
    ...
    <div class="card-actions">
        ...
        <button class="btn btn-purple btn-icon" type="button"
            ...
            onclick="openMateriasModal('{{ addslashes($fullName) }}', {{ $materiaIds }}, '{{ $materiasUrl }}')">
            <img class="img-fluid img-icon" src="{{ asset('assets/admin/EditarIcon.png') }}" style="filter: hue-rotate(120deg);">
        </button>
        ...
        <form action="{{ $deleteUrl }}" method="POST" onsubmit="return confirm('¿Seguro que deseas revocar la cuenta de {{ addslashes($fullName) }}?')">
            @csrf
            @method('DELETE')
            <x-btn-icon type="submit" id="btn-delete-{{ $profesor->id }}" icon="{{ asset('assets/admin/BorrarIcon.png') }}" />
        </form>
