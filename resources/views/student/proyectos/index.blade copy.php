...
<head>
    ...
    'resources/css/student/lista-exposiciones.css',
    ...
    <main class="flex-1 p-8 ml-0 md:ml-64 transition-all duration-300">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            @forelse($proyectos as $proyecto)
                <article
                    class="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow duration-300 border border-gray-200 overflow-visible flex flex-col h-full relative z-0 hover:z-10">
                    ...
                    <div class="bg-gray-50 px-6 py-4 border-b border-gray-100 flex justify-between items-center shrink-0">
                        ...
                        <div class="group relative cursor-help">
                            <span
                                class="px-3 py-1 rounded-full text-xs font-bold uppercase border inline-block
                            ...
                            <div
                                class="tooltip-content absolute right-0 top-full mt-2 w-64 p-3 bg-gray-800 text-white text-xs rounded-lg shadow-xl z-50 text-center pointer-events-none">
                                ...
                                <div
                                    class="absolute bottom-full right-4 -mb-1 border-4 border-transparent border-b-gray-800">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-6 flex-1 flex flex-col">
                        ...
                        <div class="mb-6 flex-1">
                            ...
                            <div class="flex items-center mb-2">
                                <h5
                                    class="text-xs font-bold uppercase tracking-wider {{ $esIndividual ? 'text-blue-600' : 'text-gray-500' }}">
                                    {{ $tituloSeccion }}
                                </h5>
                            </div>

                            <div
                                class="bg-gray-50 rounded-lg p-3 border border-gray-100 max-h-24 overflow-y-auto custom-scroll">
                                @if(!$esIndividual)
                                    <ul class="space-y-1">
                                        @foreach($proyecto->autores as $autor)
                                            <li class="flex justify-between items-center text-sm group">
                                                <span
                                                    class="text-gray-700 font-medium group-hover:text-gray-900 transition-colors {{ $autor->id == auth()->user()->estudiante->id ? 'text-blue-700' : '' }}">
                                                    {{ $autor->nombre }} {{ $autor->apellido_paterno }}
                                                </span>
                                                <span
                                                    class="font-mono text-[10px] text-gray-400 bg-white border border-gray-200 px-1.5 py-0.5 rounded">
                                                    {{ $autor->matricula }}
                                                </span>
                                            </li>
                                        @endforeach
                        ...
                        <div class="text-xs text-gray-400 mb-6 shrink-0">
                            Asignado por: <span class="font-semibold text-gray-600">Prof. {{ $proyecto->profesor->nombre }}
                                {{ $proyecto->profesor->apellido_paterno }}</span>
                        </div>
                        ...
                        <div class="mt-auto pt-4 border-t border-gray-100 shrink-0">
                            @if(in_array($proyecto->estatus, ['borrador', 'rechazado']))
                                ...
                                @if($soyLider)
                                    ...
                                    @if (!$proyecto->titulo)
                                        <a href="{{ route('estudiante.proyectos.create', $proyecto->id) }}"
                                ...
                                @else
                                    <div class="group/btn relative w-full">
                                        <button disabled
                                            class="block w-full text-center py-2.5 px-4 rounded-lg font-bold bg-gray-100 text-gray-400 border border-gray-200 cursor-not-allowed flex justify-center items-center gap-2">
                                        ...
                                        {{-- Tooltip Botón --}}
                                        <div
                                            class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 hidden group-hover/btn:block w-48 z-20">
                                            <div class="bg-gray-800 text-white text-xs rounded py-2 px-3 text-center shadow-lg">
                                                ...
                                                <div
                                                    class="absolute top-100 left-1/2 -ml-1 border-4 border-gray-800 border-t-gray-800 border-x-transparent border-b-transparent">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
            ...
            @empty
                <div
                    class="col-span-1 lg:col-span-2 text-center py-20 bg-white rounded-xl shadow-sm border border-dashed border-gray-300">
